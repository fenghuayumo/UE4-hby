...
static TAutoConsoleVariable<int32> CVarRayTracingVisualizeBVHShowLandscape(
	TEXT("r.RayTracing.VisualizeBVH.ShowFlags.LandscapeMesh"),
	0,
	TEXT("Include landscape mesh bounds when visualizing BVH"));
...
float GLandscapeRayTracingGeometryFractionalLODThreshold = 0.0f;
static FAutoConsoleVariableRef CVarLandscapeRayTracingGeometryFractionalLODThreshold(
	TEXT("r.RayTracing.Geometry.Landscape.FractionalLODThreshold"),
	GLandscapeRayTracingGeometryFractionalLODThreshold,
	TEXT("Minimal disagreement in fractional LOD to consider it different (default 0.0)\n")
...
/*
 * Snap to the next lower division of a grid.  Similar to FMath::Grid snap, but always
 * snapping to the lower value to preserve the base level.
 */
template< class T >
static FORCEINLINE T GridSnapFloor(T Location, T Grid)
{
	return (Grid == T{}) ? Location : (FMath::Floor(Location / Grid) * Grid);
}

int8 FLandscapeRenderSystem::GetLODFromScreenSize(LODSettingsComponent LODSettings, float InScreenSizeSquared, float InViewLODScale, float& OutFractionalLOD)
{
	float ScreenSizeSquared = InScreenSizeSquared / InViewLODScale;

#if RHI_RAYTRACING
	const float Threshold = FMath::Max(GLandscapeRayTracingGeometryFractionalLODThreshold, 0.0f);
#else
	...
	if (ScreenSizeSquared <= LODSettings.LastLODScreenSizeSquared)
	{
		...
		OutFractionalLOD = GridSnapFloor(OutFractionalLOD, Threshold);
...
void FLandscapeComponentSceneProxy::GetDynamicRayTracingInstances(FRayTracingMaterialGatheringContext& Context, TArray<FRayTracingInstance>& OutRayTracingInstances)
{
	...
	const bool bCheckNeighbors = GLandscapeRayTracingGeometryFractionalLODThreshold > 0.0f;
	...
	for (int32 SubY = 0; SubY < NumSubsections; SubY++)
	{
		for (int32 SubX = 0; SubX < NumSubsections; SubX++)
		{
			...
			// Detect continuous LOD parameter changes. This is for far-away high LODs - they change rarely yet the BLAS refit time is not ideal, even if they contains tiny amount of triangles
			{
				...
				if (bCheckNeighbors)
				{ 
					for (int32 NeighborIdx = 0; NeighborIdx < NEIGHBOR_COUNT; NeighborIdx++)
					{
						const FLandscapeNeighborInfo* Neighbor = GetNeighbor(NeighborIdx);

						if (Neighbor)
						{
							float NeighborLOD = RenderSystem.GetSectionLODValue(Neighbor->ComponentBase);

							if ( SectionRayTracingStates[SubSectionIdx].NeighborLOD[NeighborIdx] != NeighborLOD)
							{
								bNeedsRayTracingGeometryUpdate = true;
								SectionRayTracingStates[SubSectionIdx].NeighborLOD[NeighborIdx] = NeighborLOD;
							}
						}
					}
				}
			...
			if (Context.bGatherAABB && CVarRayTracingVisualizeBVHShowLandscape->GetBool())
			{
