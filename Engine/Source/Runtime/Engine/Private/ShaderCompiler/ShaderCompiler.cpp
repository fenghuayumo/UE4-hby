	...
	)
{
	...
	//Hybrid Translucency support
	{
		bool bSupportHybridTranslucency = false;
#if RHI_RAYTRACING
		static const auto CVarHybridTranslucency = IConsoleManager::Get().FindTConsoleVariableDataInt(TEXT("r.RayTracing.HybridTranslucencySupport"));

		const bool bPlatformSupportsRayTracing = RHISupportsRayTracingShaders(static_cast<EShaderPlatform>(Target.Platform));
		const bool bHybridTranslucency = CVarHybridTranslucency ? (CVarHybridTranslucency->GetValueOnAnyThread() != 0) : false;

		bSupportHybridTranslucency = bPlatformSupportsRayTracing && bHybridTranslucency;
#endif
		Input.Environment.SetDefine(TEXT("HYBRIDTRANSLUCENCY"), (Target.Frequency == SF_Pixel) && bSupportHybridTranslucency);
	}
