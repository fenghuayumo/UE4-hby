...
BEGIN_GLOBAL_SHADER_PARAMETER_STRUCT(FPrimitiveUniformShaderParameters,ENGINE_API)
    ...
    SHADER_PARAMETER(uint32, OutputVelocityAndVisibleInRayTracing)
...
)
{
	...
	Result.OutputVelocityAndVisibleInRayTracing = (bOutputVelocity) ? 1 : 0;
	Result.OutputVelocityAndVisibleInRayTracing |= (bVisibleInRayTracing) ? 2 : 0;
...
)
{
	...
	return GetPrimitiveUniformShaderParameters(LocalToWorld, PreviousLocalToWorld, ActorPosition, WorldBounds, LocalBounds, LocalBounds, bReceivesDecals, bHasDistanceFieldRepresentation, bHasCapsuleRepresentation, 
		bUseSingleSampleShadowFromStationaryLights, bUseVolumetricLightmap, bDrawsVelocity, LightingChannelMask, LpvBiasMultiplier, LightmapDataIndex, SingleCaptureIndex, bOutputVelocity, bVisibleInRayTracing, nullptr, bCastContactShadow);
}
	...
	)
{
	...
	return TUniformBufferRef<FPrimitiveUniformShaderParameters>::CreateUniformBufferImmediate(
		GetPrimitiveUniformShaderParameters(LocalToWorld, LocalToWorld, WorldBounds.Origin, WorldBounds, LocalBounds, PreSkinnedLocalBounds, bReceivesDecals, false, false, false, false, bDrawsVelocity, GetDefaultLightingChannelMask(), LpvBiasMultiplier, INDEX_NONE, INDEX_NONE, false, false, nullptr),
