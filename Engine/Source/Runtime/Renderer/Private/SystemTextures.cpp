...
void FSystemTextures::InitializeFeatureLevelDependentTextures(FRHICommandListImmediate& RHICmdList, const ERHIFeatureLevel::Type InFeatureLevel)
{
	...
	// Create a TextureArrayBlackDummy texture
	if (CurrentFeatureLevel < ERHIFeatureLevel::SM5 && InFeatureLevel >= ERHIFeatureLevel::SM5)
	{
		FPooledRenderTargetDesc Desc(FPooledRenderTargetDesc::Create2DDesc(FIntPoint(1, 1), PF_FloatRGBA, FClearValueBinding::Transparent, TexCreate_HideInVisualizeTexture, TexCreate_ShaderResource | TexCreate_NoFastClear, false));
		Desc.AutoWritable = false;
		Desc.ArraySize = 2;
		GRenderTargetPool.FindFreeElement(RHICmdList, Desc, TextureArrayBlackDummy, TEXT("TextureArrayBlackDummy"), ERenderTargetTransience::NonTransient);
	}
...
void FSystemTextures::ReleaseDynamicRHI()
{
	...
	GTAOPreIntegrated.SafeRelease();
	TextureArrayBlackDummy.SafeRelease();
