...
#if RHI_RAYTRACING
	bool bHybridTranslucency = ShouldRenderRayTracingHybridTranslucency();
	if (!bHybridTranslucency)
	{
...
#if RHI_RAYTRACING
	static const auto HybridTranslucencyDepthThresholdCVar = IConsoleManager::Get().FindTConsoleVariableDataFloat(TEXT("r.RayTracing.HybridTranslucency.DepthThreshold"));
	static const auto HybridTranslucencyHalfResCVar = IConsoleManager::Get().FindTConsoleVariableDataInt(TEXT("r.RayTracing.HybridTranslucency.HalfRes"));

	if (ShouldRenderRayTracingTranslucency(View) && ShouldRenderRayTracingHybridTranslucency())
	{
		...
		BasePassParameters.RayTracingTTranslucencyUpscale = HybridTranslucencyHalfResCVar ? HybridTranslucencyHalfResCVar->GetValueOnRenderThread() : 0;
...
void FDeferredShadingSceneRenderer::RenderTranslucency(
	...
	ETranslucencyView ViewsToRender)
{
	if (!EnumHasAnyFlags(ViewsToRender, ETranslucencyView::UnderWater | ETranslucencyView::AboveWater | ETranslucencyView::RayTracingMask))
	{
		return;
	}
