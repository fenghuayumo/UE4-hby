...
int32 GRayTracingExcludeTranslucentsFromShadows = 0;
static FAutoConsoleVariableRef CVarEnableRayTracingExcludeTranslucentsFromShadows(
	TEXT("r.RayTracing.ExcludeTranslucentsFromShadows"),
	GRayTracingExcludeTranslucentsFromShadows,
	TEXT(" 0: obey material flags for casting shadows (default)\n")
	TEXT(" 1: override shadow casting to off for all translucency materials\n"),
	ECVF_RenderThreadSafe
);

bool FRayTracingMeshProcessor::ExcludeTranslucentsFromShadows()
{
	return GRayTracingExcludeTranslucentsFromShadows != 0;
}
...
)
{
	...
	check(RayTracingMissShaderLibrary.Num() == RAY_TRACING_MISS_SHADER_SLOT_DEFAULT);
	RayTracingMissShaderLibrary.Add(DefaultMissShader);
	
	check(RayTracingMissShaderLibrary.Num() == RAY_TRACING_MISS_SHADER_SLOT_LIGHTING);
	RayTracingMissShaderLibrary.Add( bLightingMissShader ? GetRayTracingLightingMissShader(View) : DefaultMissShader);
	
	PrepareLightFunctionMissShaders(LightFunctionLights, View, RayTracingMissShaderLibrary);
