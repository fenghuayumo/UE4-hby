...
class FRayTracingPrimaryRaysRGS : public FGlobalShader
{
	...
	class FHybridTranslucency : SHADER_PERMUTATION_BOOL("DIM_HYBRID_TRANSLUCENCY");

	using FPermutationDomain = TShaderPermutationDomain<FDenoiserOutput, FEnableTwoSidedGeometryForShadowDim, FMissShaderLighting, FHybridTranslucency>;
...
void FDeferredShadingSceneRenderer::PrepareRayTracingTranslucency(const FViewInfo& View, TArray<FRHIRayTracingShader*>& OutRayGenShaders)
{
	...
	if (GetRayTracingTranslucencyOptions(View).bEnabled || View.RayTracingRenderMode == ERayTracingRenderMode::RayTracingDebug)
	{
		...
		PermutationVector.Set<FRayTracingPrimaryRaysRGS::FHybridTranslucency>(ShouldRenderRayTracingHybridTranslucency());
	...
	ERayTracingPrimaryRaysFlag Flags)
{
	...
	else
	{
		...
		PassParameters->LayersColor = nullptr;
		PassParameters->LayersDepth = nullptr;
	}
