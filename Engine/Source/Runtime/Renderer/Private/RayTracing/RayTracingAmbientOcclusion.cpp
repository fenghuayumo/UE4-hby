...
static TAutoConsoleVariable<int32> CVarRayTracingAmbientOcclusionTiming(
	TEXT("r.RayTracing.AmbientOcclusion.Timing"),
	1,
	TEXT("Time cost of ray traced AO ( 0 - off, 1 - on)"),
	ECVF_RenderThreadSafe);

static TAutoConsoleVariable<int32> CVarRayTracingAmbientOcclusionUseBiasForSkipWPOEval(
	TEXT("r.RayTracing.AmbientOcclusion.UseBiasForSkipWPOEval"),
	0,
	TEXT("Bias applied to minimum ray distance to hide mismatched geometry for materials that use WPO but skip evaluating it\n"),
	ECVF_RenderThreadSafe);

static TAutoConsoleVariable<float> CVarRayTracingAmbientOcclusionMaxBiasForInexactGeometry(
	TEXT("r.RayTracing.AmbientOcclusion.MaxBiasForInexactGeometry"),
	0.8,
...
class FRayTracingAmbientOcclusionRGS : public FGlobalShader
{
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER(int32, UseBiasForSkipWPOEval)
...
#if RHI_RAYTRACING
{
	...
	PassParameters->MaxBiasForInexactGeometry = CVarRayTracingAmbientOcclusionMaxBiasForInexactGeometry.GetValueOnRenderThread();
	PassParameters->UseBiasForSkipWPOEval = CVarRayTracingAmbientOcclusionUseBiasForSkipWPOEval.GetValueOnRenderThread() != 0;
	...
	if (VisualizeRayTracingTiming(View) && CVarRayTracingAmbientOcclusionTiming.GetValueOnRenderThread() != 0)
	{
	...
	else
	{
		...
		PassParameters->CumulativeTime = GraphBuilder.CreateUAV(DenoiserInputs.RayHitDistance);
	}
