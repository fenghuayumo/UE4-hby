...
static TAutoConsoleVariable<float> CVarRayTracingShadowMaxBiasForInexactGeometry(
	TEXT("r.RayTracing.Shadow.MaxBiasForInexactGeometry"),
	0.8f,
	TEXT("Bias applied to minimum ray distance to hide mismatched geometry for following materials:\n")
	TEXT("Materials that are using dithered LOD\n")
	TEXT("Materials that are skipping WPO evaluation for performance reasons (If UseBiasForSkipWPOEval=1)\n")
	TEXT("Instanced meshes that do not evaluate positions\n"),
	ECVF_RenderThreadSafe
);

static TAutoConsoleVariable<int32> CVarRayTracingShadowUseBiasForSkipWPOEval(
	TEXT("r.RayTracing.Shadow.UseBiasForSkipWPOEval"),
	0,
	TEXT("Enable bias for meshes (e.g. foliage) that are not evaluating WPO to prevent shadow artifacts ( 0 - off, 1 - on )\n"),
	ECVF_RenderThreadSafe
);

static TAutoConsoleVariable<int32> CVarRayTracingShadowsSupportTranslucency(
	TEXT("r.RayTracing.Shadows.SupportTranslucency"),
	0,
	TEXT("Enable to evaluate translucent geometry marked as shadow casting when tracing shadows"),
	ECVF_RenderThreadSafe
);

static TAutoConsoleVariable<int32> CVarRayTracingShadowsTiming(
	TEXT("r.RayTracing.Shadows.Timing"),
	1,
	TEXT("Time cost of ray traced shadows ( 0 - off, 1 - on)"),
	ECVF_RenderThreadSafe);

static TAutoConsoleVariable<int32> CVarRayTracingShadowOptimizeMultipleSampleEval(
	TEXT("r.RayTracing.Shadow.OptimizeMultipleSampleEval"),
	0,
	TEXT("Skip evaluating multiple samples per pixel when all threads in a warp have the same ray hit predicate\n")
	TEXT("This assumes that sampled threads in a warp are spatially local and if these results are the same, likely so are the subsamples\n")
	TEXT("This can improve performance at the cost of some quality + correctness\n"),
	ECVF_RenderThreadSafe
);
...
class FOcclusionRGS : public FGlobalShader
{
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER(uint32, SupportTranslucency)
		SHADER_PARAMETER(uint32, bOptimizeMultipleSampleEval)
...
#if RHI_RAYTRACING
{
	...
	// Ray generation pass for shadow occlusion.
	{
		...
		PassParameters->MaxBiasForInexactGeometry = CVarRayTracingShadowMaxBiasForInexactGeometry.GetValueOnRenderThread();
		PassParameters->SupportTranslucency = CVarRayTracingShadowsSupportTranslucency.GetValueOnRenderThread();
		PassParameters->bOptimizeMultipleSampleEval = RayTracingConfig.RayCountPerPixel > 1 ? CVarRayTracingShadowOptimizeMultipleSampleEval.GetValueOnRenderThread() : 0;

		if (VisualizeRayTracingTiming(View) && CVarRayTracingShadowsTiming.GetValueOnRenderThread() != 0)
		{
		...
		else
		{
			...
			PassParameters->CumulativeTime = OutRayHitDistanceUAV; // bogus UAV to just keep validation happy as it is dynamically unused
		}
		...
		if (bForceSharp)
		{
			PassParameters->Light.SourceRadius = 0.0f;
			PassParameters->Light.SourceLength = 0.0f;
		}
