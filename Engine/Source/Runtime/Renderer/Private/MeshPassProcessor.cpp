...
int32 FMeshDrawShaderBindings::GatherBoundParameters(FReadOnlyMeshDrawSingleShaderBindings& SingleShaderBindings, const int32 MaxUniformBuffers, const FRHIUniformBuffer** LocalUniformBuffers)
{
	FRHIUniformBuffer* const* RESTRICT UniformBufferBindings = SingleShaderBindings.GetUniformBufferStart();
	const FShaderParameterInfo* RESTRICT UniformBufferParameters = SingleShaderBindings.ParameterMapInfo.UniformBuffers.GetData();
	const int32 NumUniformBufferParameters = SingleShaderBindings.ParameterMapInfo.UniformBuffers.Num();
	...
	uint32 ShaderSlot) const
{
	...
	int32 MaxUniformBufferUsed = GatherBoundParameters(SingleShaderBindings, UE_ARRAY_COUNT(BindingState.UniformBuffers), (const FRHIUniformBuffer **)BindingState.UniformBuffers);

	checkf(SingleShaderBindings.ParameterMapInfo.LooseParameterBuffers.Num() == 0, TEXT("Ray tracing callable shaders may not have loose parameters"));

	uint32 NumUniformBuffersToSet = MaxUniformBufferUsed + 1;
	const uint32 UserData = 0; // UserData could be used to store material ID or any other kind of per-material constant. This can be retrieved in hit shaders via GetHitGroupUserData().
	RHICmdList.SetRayTracingMissShader(Scene, ShaderSlot, PipelineState, HitGroupIndex,
		NumUniformBuffersToSet, (FRHIUniformBuffer**)BindingState.UniformBuffers,
		UserData);
}
...
void FMeshDrawShaderBindings::Initialize(TMaterialProcessorShaders<ShaderType> Shaders)
{
...
template void FMeshDrawShaderBindings::Initialize<FMeshMaterialShader>(TMaterialProcessorShaders<FMeshMaterialShader> Shaders);
template void FMeshDrawShaderBindings::Initialize<FMaterialShader>(TMaterialProcessorShaders<FMaterialShader> Shaders);
