...
#undef DECLARE_SHADER_TRAITS
	...
	template <ED3D12PipelineType PipelineType>
	D3D12_STATE_CACHE_INLINE void InternalSetDescriptorHeaps()
	{
		static_assert(PipelineType != D3D12PT_RayTracing, "FD3D12StateCacheBase is not expected to be used with ray tracing.");

		if (bNeedSetDescriptorHeaps)
		{
			...
			// that's set for the duration of RT dispatch, and we restore the heap every time we transition from RT work.
			ID3D12DescriptorHeap* CurrentHeaps[] =
			{
				DescriptorCache.GetCurrentViewHeap()->GetHeap(),
				DescriptorCache.GetCurrentSamplerHeap()->GetHeap(),
			};

			CmdContext->CommandListHandle.GraphicsCommandList()->SetDescriptorHeaps(2, CurrentHeaps);

			bNeedSetDescriptorHeaps = false;
		}
	}
...
public:
	...
	void TransitionComputeState(ED3D12PipelineType PipelineType)
	{
		if (LastComputePipelineType != PipelineType)
		{
			...
			if (LastComputePipelineType == D3D12PT_RayTracing)
			{
				bNeedSetDescriptorHeaps = true;
			}
	...
	void TransitionGraphicsState()
	{
		...
		LastComputePipelineType = D3D12PT_Graphics;
	}
