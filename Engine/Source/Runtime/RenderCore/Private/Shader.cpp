...
#if RHI_RAYTRACING
	if (RHISupportsRayTracingShaders(Platform))
	{
		static const auto CVar = IConsoleManager::Get().FindConsoleVariable(TEXT("r.RayTracing.HybridTranslucencySupport"));
		if (CVar && CVar->GetInt() != 0)
		{
			KeyString += TEXT("_hybTrans");
		}
	}
#endif
