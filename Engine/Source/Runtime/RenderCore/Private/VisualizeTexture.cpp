...
class FVisualizeTexturePS : public FGlobalShader
{
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER_RDG_TEXTURE(Texture2DArray, VisualizeTexture2DArray)
...
static EVisualisePSType GetVisualizePSType(const FRDGTextureDesc& Desc)
{
	if(Desc.IsTexture2D())
	{
		...
		else
		{
			...
			{
				return EVisualisePSType::Texture2DArray;
			}
			else
			{
				// non MSAA
...
void FVisualizeTexture::CreateContentCapturePass(FRDGBuilder& GraphBuilder, const FRDGTextureRef InputTexture, uint32 CaptureId)
{
...

	{
...

		{
			...
			PassParameters->VisualizeTexture2DArray = InputTexture;
		...
		if (GetEmitRDGEvents())
		{
			...
			if (InputDesc.IsTexture2D() && InputDesc.IsTextureArray())
			{
				ExtendedDrawEvent += FString::Printf(TEXT("x%d CapturedSlice=%d"), InputDesc.ArraySize, Config.ArrayIndex);
			}
