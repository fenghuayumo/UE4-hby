...
NVSDK_NGX_Result FNGXD3D11RHI::Init_NGX_D3D11(const FNGXRHICreateArguments& InArguments, const wchar_t* InApplicationDataPath, ID3D11Device* InHandle, const NVSDK_NGX_FeatureCommonInfo* InFeatureInfo)
{
	...
	// Workaround for NGX checking for a matching interface instead of the underlying object later on in NGX_D3D11_CREATE_DLSS_EXT()
	// This issue shows up if something substitutes a proxy device interface. Has been seen with EOS plugin.
	// The issue will be fixed in a future NGX release, then this workaround can be retired
#define NGX_DEVICE_INTERFACE_WORKAROUND 1
#if NGX_DEVICE_INTERFACE_WORKAROUND
	ID3D11DeviceContext* InnerCtx;
	ID3D11Device* InnerDevice;
	InHandle->GetImmediateContext(&InnerCtx);
	InnerCtx->GetDevice(&InnerDevice);
	ID3D11Device* Device = InnerDevice;
#else
	ID3D11Device* Device = InHandle;
#endif
	...
	do 
	{
		...
		{
			Result = NVSDK_NGX_D3D11_Init(InArguments.NGXAppId, InApplicationDataPath, Device, InFeatureInfo, static_cast<NVSDK_NGX_Version>(APIVersion));
		...
		{
			Result = NVSDK_NGX_D3D11_Init_with_ProjectID(TCHAR_TO_UTF8(*InArguments.UnrealProjectID), NVSDK_NGX_ENGINE_TYPE_UNREAL, TCHAR_TO_UTF8(*InArguments.UnrealEngineVersion), InApplicationDataPath, Device, InFeatureInfo, static_cast<NVSDK_NGX_Version>(APIVersion));
